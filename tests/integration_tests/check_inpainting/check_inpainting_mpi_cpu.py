...
def test_check_distributed():
    ...
    analyze_data(
        ...
        params["savePath"],
        params["dataPath"],
        "results_inpainting_mpi-cpu",
        comm_size=comm_size,
        show_results=False,
    )

    ref_file = open("../data/reference_inpainting_mpi-cpu.json")
    ref = json.load(ref_file)

    snr_ref = ref["snr_recons"]
    ssim_ref = ref["ssim_recons"]

    results_file = open(join(params["savePath"], "results_inpainting_mpi-cpu.json"))
