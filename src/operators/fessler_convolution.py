...
from dsgs.operators.padding import crop_array_nd, pad_array_nd
...
class SerialConvolution(LinearOperator):
    ...
    def __init__(
        ...
        """
        if not len(kernel.shape) == data_size.size:
            raise ValueError("kernel should have ndims = len(data_size) dimensions")

        self.overlap_size = np.array(kernel.shape, dtype="i") - 1
        image_size = data_size + self.overlap_size
        # TODO: change the sizes and change of conventions ins this case
        ...
        self.conv_size = self.image_size + self.overlap_size
        self.kernel = kernel
    ...
    def forward(self, input_image):
        ...
        """
        return crop_array_nd(
            fft_conv(input_image, self.fft_kernel, self.conv_size),
            self.overlap_size,
            self.overlap_size,
        )
    ...
    def adjoint(self, input_data):
        ...
        """
        return fft_conv(
            pad_array_nd(input_data, self.overlap_size, self.overlap_size),
            np.conj(self.fft_kernel),
            self.conv_size,
