"""Synchronous MPI-distributed linear convolution operator."""
...
# FIXME: see if same convolution kernel (and size) can be used for both the direct and the adjoint operator (not 100% sure, old version was using sizes for the direct operator for both operations)

# ? Keep kernel value out of the associated model object?
# ? change interface of forward / adjoint method to pass the output array, to be updated in place? (would need to do the same for the serial case)
...
class SyncLinearConvolution(LinearOperator):
    ...
    def forward(self, input_image):
        ...
        Note
        ----
        The input buffer ``input_image`` is copied inside forward_buffer. This intern buffer will be used for the communications and the computations.
        """
    ...
    def adjoint(self, input_data):
        ...
        Note
        ----
        The input is copied inside adjoint_buffer. This intern buffer will be used for the communications and the computations.
        """
