...
def chunk_gradient_2d(x, rank, islast):
    ...
    Returns
    -------
    u : cupy.ndarray[float64 or complex128], 2d
    ...
    with cp.cuda.Device(rank):
        ...
        u = cp.zeros((2, local_shape0, local_shape1), dtype=x.dtype)
...
class distributed_gradient2d:
    def __init__(self, global_size: np.ndarray, grid_size: np.ndarray) -> None:
        ...
        with cp.cuda.Device(self.rank):
            ...
            self.local_buffer_adj_v = cp.zeros(
                self.adj_cart_comm_v.cartslicer.facet_size
            )
            self.local_buffer_adj_h = cp.zeros(
