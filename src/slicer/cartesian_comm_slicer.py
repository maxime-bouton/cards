...
from slicer.cartesian_tesselation import local_split_range_nd
from slicer.comm_slicer import CommSlicer

# TODO;
# write down notes in my notebook (math form -> in LaTeX)
# !TODO

# TODO: see methods to update a CartesianSlicer (e.g., if redefining
# size, ...), insist on what should not be accessed

# TODO: extend communicator / send_rank and slicer to accommodate circular communications
...
def compute_local_buffer_size(
    ...
    if backward:
        facet_size = (
            tile_size + np.bitwise_and((ranknd > 0), (grid_size > 1)) * overlap_size
        )
    else:
        facet_size = (
            tile_size
            + np.bitwise_and((ranknd < grid_size - 1), (grid_size > 1)) * overlap_size
        )

    return facet_size.astype(int)


def create_slice_buffers(ranknd, grid_size, tile_size, overlap_size, backward=True):
    r"""Create a slice to extract tile (elements exclusively handled by the current process) from the associated facet (gathering both the tile and the elements received by the current process from neighbour processes).
    ...
    backward : bool, optional
        Direction of the overlap between two consecutive processes along all axes, by default True.
    ...
    if backward:
        sel_size = np.zeros(ndims, dtype="i")
        cond2 = ranknd > 0
        sel_size[cond2] = overlap_size[cond2]
        # ! if axis not split, select full axis
        cond1 = grid_size <= 1
        sel_size[cond1] = 0
        slice_ = tuple([np.s_[sel_size[d] :] for d in range(ndims)])
    else:
        slice_ = tuple([np.s_[: tile_size[d]] for d in range(ndims)])
...
def create_slice_sync_send_recv(
    ...
    r"""Slice to extract received and sent elements from a worker (sync).

    Create slice to extract elements received or sent by a worker from the
    facet buffer during the synchronous communication phases occuring along each axis of a Cartesian grid of processes.
...
def create_slice_async_send_recv(
    ...
    r"""Slice to extract received and sent elements from a worker (async).
...
class CartesianCommSlicer(CommSlicer):
    r"""Slicer object underlying Cartesian communications.
    ...
    def __init__(
        ...
        r"""Generate slices to manipulate sub-arrays involved in the Cartesian decomposition of a global array.
        ...
        if backward:
            self.send_size[np.bitwise_or(grid_size <= 1, ranknd == grid_size - 1)] = 0
            self.recv_size[np.bitwise_or(grid_size <= 1, ranknd == 0)] = 0
        else:
            self.send_size[np.bitwise_or(grid_size <= 1, ranknd == 0)] = 0
            self.recv_size[np.bitwise_or(grid_size <= 1, ranknd == grid_size - 1)] = 0
        ...
        if tile_range is None:
            self.tile_range = local_split_range_nd(
                grid_size, global_buffer_size, self.ranknd
            )
        else:
    ...
    def _get_slice_global_buffer_to_tile(self):
        r"""Create slice to insert tile into, or extract tile from, the full
        array."""
        # global_buffer[slice_local_range] = local_tile
        return tuple(
            [
                np.s_[self.tile_range[d, 0] : self.tile_range[d, 1] + 1]
